...
if __name__ == '__main__':
    ...
    env, last_observation, lives = env_init(env_name=ENV_NAME, 
                                            frame_skip=FRAMESKIP, 
                                            screen_size=OBSERVATION_HEIGHT_WIDTH)

    # Context deques matching STORM train.py (maxlen=16)
    ...
    for epoch in range(EPOCHS):
        ...
        dist_head.eval()
        actor.eval()
        observations, actions, rewards, terminations, last_observation, lives = gather_steps(env=env, 
                                                                                                observation=last_observation, 
                                                                                                lives=lives,
                                                                                                env_steps_per_epoch=ENV_STEPS_PER_EPOCH, 
                                                                                                actor=actor, 
